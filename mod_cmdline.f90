...
logical(kind=lk) :: extract_comm_output_on = .false._lk
...
integer(kind=ik)                           :: cmd_nn,cmd_nmax
 ...
 enddo
  if(extract_comm_output_on) then
       write(*,'(a,a,a)') 'test_all_keywords: "',&
...
if(nmax==0) call find_command_keywords(syntax,keywords,nmax) ! called only the first time
 ...
 call scan_for_keyword(switch,keywords,nmax,nn_switch,1_ik,ind_end)
...
!*****************************************************************************************************
function extract_command_parameter_real(command_line,switch,stop_on_error,value,syntax) &
  result(result_value)
integer(kind=ik)           :: result_value
logical(kind=lk),optional  :: stop_on_error
logical(kind=lk)           :: loc_stop_on_error
...
if(nn_switch==0) then
 ...
 if(ind==0) then
    if(loc_stop_on_error) then
      if(extract_comm_output_on) then
       write(*,'(a,a,a)') 'extract_command_parameter_real: did not found "',&
        trim(switch),'" in command line'
       write(*,'(a)') trim(command_line)
       write(*,'(a)') 'syntax of command line is'
       write(*,'(a)') trim(syntax)
...
!*****************************************************************************************************
function extract_command_parameter_integer(command_line,switch,stop_on_error,value,syntax) &
  result(result_value)
integer(kind=ik)           :: result_value
logical(kind=lk),optional  :: stop_on_error
logical(kind=lk)           :: loc_stop_on_error
 ...
 if(ind==0) then
    ...
    if(status/=0) then
       if(loc_stop_on_error) then
        if(extract_comm_output_on) then
            write(*,'(3a)') &
              'extract_command_parameter_integer: error reading integer(kind=ik) parameter for switch "'&
              ,trim(switch),'"'
            write(*,'(3a)') 'found no integer(kind=ik) in "',command_line(ind+lensw:ind_end),'"'
          end if
          stop 'extract_command_parameter_integer: error reading integer(kind=ik) parameter for switch'
       endif
    endif
    if(extract_comm_output_on) then
